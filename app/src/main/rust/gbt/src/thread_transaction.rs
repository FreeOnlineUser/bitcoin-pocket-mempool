...
impl ThreadTransaction {
    ...
    pub fn from_jni(env: &mut JNIEnv, obj: &JObject) -> jni::errors::Result<Self> {
        ...
        // Get inputs array
        let inputs_obj = env.get_field(obj, "inputs", "[I")?.l()?;
        let inputs_array: JIntArray = inputs_obj.into();
        let inputs_len = env.get_array_length(&inputs_array)?;
        let mut inputs_buf = vec![0i32; inputs_len as usize];
        env.get_int_array_region(&inputs_array, 0, &mut inputs_buf)?;
